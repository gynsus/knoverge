...
export class KnowledgeService {
  ...
  async create(actor: ActorContext, input: CreateItemInput): Promise<ItemResult> {
    ...
    return this.o.crossStore.run<ItemResult>(actor, {
      ...
      commit: async (operation) => {
        ...
        await this.assertSummaryOf(actor.workspaceId, itemId, draft);
      ...
      record: async (tx, operation) => {
        ...
        return {
          ...
          body: p.body,
          stale: await this.staleAfter(tx, actor.workspaceId, item),
        };
      },
    });
  }
    ...
    return this.o.crossStore.run<ItemResult>(actor, {
      ...
      commit: async (operation) => {
        ...
        const draft: Frontmatter = {
          ...
          ...(input.summaryOf ? { summary_of: [...input.summaryOf] } : {}),
        ...
        if (draft.type !== 'summary') delete (draft as { summary_of?: unknown }).summary_of;
        await this.assertSummaryOf(actor.workspaceId, input.itemId, draft);
      ...
      record: async (tx, operation) => {
        ...
        const after = (await this.o.items.findById(actor.workspaceId, input.itemId, tx)) as never;
        return {
          item: after,
          revision,
          categories: p.chosen.map((c) => c.path),
          tags: p.frontmatter.tags,
          body: p.body,
          stale: await this.staleAfter(tx, actor.workspaceId, after),
        };
      },
    });
  }
  ...
  async supersede(actor: ActorContext, input: SupersedeInput): Promise<SupersedeResult> {
    ...
    return this.o.crossStore.run<SupersedeResult>(actor, {
      ...
      commit: async (operation) => {
        ...
        await this.assertSummaryOf(actor.workspaceId, newItemId, newDraft);
      ...
      record: async (tx, operation) => {
        ...
        await this.writeSummaryOf(tx, newItemId, pNew.frontmatter);
        await this.recordDisputes(tx, actor, disputed.partners, operation.id);
        await this.index(
          tx,
          actor.workspaceId,
        ...
        const replacementRow = (await this.o.items.findById(
          actor.workspaceId,
          newItemId,
          tx,
        )) as never;
        const supersededRow = (await this.o.items.findById(
          actor.workspaceId,
          input.oldItemId,
          tx,
        )) as never;
        return {
          new: {
            ...
            body: pNew.body,
            stale: await this.staleAfter(tx, actor.workspaceId, replacementRow),
          },
          old: {
            item: supersededRow,
            ...
            tags: old.tags,
            body: pOld.body,
            stale: await this.staleAfter(tx, actor.workspaceId, supersededRow),
          },
        };
      },
    });
  ...
  async get(
    ...
    return {
      ...
      // Asked only of a summary: nothing else has dependencies to be out of
      // step with, and the query is a join nobody else needs.
      stale:
        item.type === 'summary' &&
        (await this.o.summaries.staleAmong(actor.workspaceId, [item.id])).has(item.id),
    };
  }
  ...
  async list(actor: ActorContext, options: ListItemsOptions = {}): Promise<ItemSummary[]> {
    ...
    // One query for the whole page, not one per summary (ADR 0024).
    const stale = await this.o.summaries.staleAmong(
      actor.workspaceId,
      items.filter((item) => item.type === 'summary').map((item) => item.id),
    );
    ...
    return this.o.crossStore.run<ItemResult>(actor, {
      ...
      record: async (tx, operation) => {
        ...
        return {
          ...
          tags: p.frontmatter.tags,
          body: p.body,
          // A restored summary can be out of step already: its sources moved on
          // while it was out of the index.
          stale: await this.staleAfter(tx, actor.workspaceId, back),
        };
      },
    });
  }
  ...
  /**
   * Refuses a `summary_of` that names something the workspace cannot answer for.
   *
   * The foreign keys would catch most of it, as an internal error on the way
   * out. A caller who named the wrong revision deserves to be told which, and
   * told it before anything reaches Git.
   */
  private async assertSummaryOf(
    workspaceId: WorkspaceId,
    itemId: KnowledgeItemId,
    frontmatter: Frontmatter,
  ): Promise<void> {
    const refs = frontmatter.summary_of ?? [];
    if (refs.length === 0) return;
    if (frontmatter.type !== 'summary') {
      throw new DomainError('VALIDATION_ERROR', 'only an item of type summary summarises anything');
    }
    const named = new Set<string>();
    for (const ref of refs) {
      const { itemId: sourceId, revisionId } = splitDependencyRef(ref);
      if (sourceId === itemId) {
        throw new DomainError('VALIDATION_ERROR', 'a summary cannot summarise itself');
      }
      if (named.has(sourceId)) {
        throw new DomainError(
          'VALIDATION_ERROR',
          `${sourceId} is named twice; a summary cannot say which revision of it it used`,
          { objectIds: { knowledge_item: sourceId } },
        );
      }
      named.add(sourceId);
      const revision = await this.o.revisions.findById(workspaceId, revisionId);
      if (!revision) {
        throw new DomainError('NOT_FOUND', `no revision ${revisionId}`, {
          objectIds: { revision: revisionId },
        });
      }
      ...
      if (false) {
        throw new DomainError(
          ...
          { objectIds: { knowledge_item: sourceId, revision: revisionId } },
        );
      }
    }
  }
  ...
  /**
   * Whether a written summary is already out of step with what it named.
   *
   * Asked inside the write's own transaction, because the dependency rows it is
   * about were written there. A caller may legitimately name an older revision —
   * you summarise what you read, and it may have moved on while you wrote — so
   * this is not something a write can assume away.
   */
  private async staleAfter(
    tx: Tx,
    workspaceId: WorkspaceId,
    item: Pick<KnowledgeItemRecord, 'id' | 'type'>,
  ): Promise<boolean> {
    if (item.type !== 'summary') return false;
    return (await this.o.summaries.staleAmong(workspaceId, [item.id], tx)).has(item.id);
  }

  /** What the summary says it was made from, as the queryable index (rule 2). */
  private async writeSummaryOf(
    tx: Tx,
    itemId: KnowledgeItemId,
    frontmatter: Frontmatter,
  ): Promise<void> {
    await this.o.summaries.replaceForSummary(
      tx,
      itemId,
      (frontmatter.summary_of ?? []).map((ref, position) => {
        const { itemId: sourceItemId, revisionId } = splitDependencyRef(ref);
        return { sourceItemId, sourceRevisionId: revisionId, position };
      }),
    );
  }
