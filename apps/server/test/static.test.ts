...
function app(dist?: string) {
  const instance = buildApp({ version: '0', probes, ...(dist ? { webDist: dist } : {}) });
  apps.push(instance);
  return instance;
}
...
describe('static web bundle', () => {
  it('serves index.html at /', async () => {
    const res = await app(webDist).inject({ method: 'GET', url: '/' });
  ...
  it('serves asset files', async () => {
    const res = await app(webDist).inject({ method: 'GET', url: '/app.js' });
    expect(res.statusCode).toBe(200);
    expect(res.body).toBe('console.log(1)');
  });

  it('falls back to index.html for client-side routes', async () => {
    const res = await app(webDist).inject({ method: 'GET', url: '/review/inbox' });
    expect(res.statusCode).toBe(200);
    expect(res.body).toContain('id="root"');
  });

  it('never lets index.html be cached but marks hashed assets immutable', async () => {
    const instance = app(webDist);
  ...
  it('does not fall back for non-GET requests', async () => {
    const res = await app(webDist).inject({ method: 'POST', url: '/anything' });
