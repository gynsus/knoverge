...
2. Each event stores `prev_event_hash` and `event_hash`, where
   ...
   `canonical_json` is RFC 8785 (JSON Canonicalization Scheme) over every stored column except `event_hash`, with timestamps in RFC 3339 UTC and `null` for absent values. The first event of a workspace uses `prev_event_hash = HMAC-SHA256(KNOVERGE_LEDGER_KEY, "")`.
